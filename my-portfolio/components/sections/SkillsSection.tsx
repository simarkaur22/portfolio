...
const iconMap: Record<string, React.ReactNode> = {
  "Backend & API Architecture (Primary)": <Server className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />,
  "Databases & Cloud": <Database className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />,
  "Frontend Development": <Code2 className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />,
  "eCommerce Solutions": <ShoppingBag className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />,
  "AI & Developer Tools": <Cpu className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
};
...
export const SkillsSection: React.FC = () => {
  ...
  return (
    <section id="skills" className="py-16 sm:py-24 relative z-10 bg-[var(--bg-page)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 space-y-4">
          <Badge variant="cyan" size="md" icon={<Sparkles className="w-3.5 h-3.5 shrink-0" />}>
            Technical Expertise
          </Badge>
        ...
        {/* Category Tabs */}
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 mb-10 sm:mb-12 max-w-full">
          {skillCategories.map((cat, idx) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-3.5 sm:px-5 py-2.5 sm:py-3 rounded-xl text-xs sm:text-sm font-semibold transition-all duration-300 cursor-pointer min-h-[42px] ${
                  isActive
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="text-center max-w-xl mx-auto px-2">
              <h3 className="text-lg sm:text-xl font-bold text-[var(--text-heading)] mb-2">
                  ...
                  <GlassCard key={skill.name} className="p-4 flex flex-col justify-between space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 truncate">
                        ...
                        <span className="text-sm font-bold text-[var(--text-heading)] truncate">{skill.name}</span>
                      </div>
                      {skill.badge && (
