...
export const ExperienceSection: React.FC = () => {
  return (
    <section id="experience" className="py-16 sm:py-24 relative z-10 bg-[var(--bg-page)]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        
        {/* Section Header */}
        <div className="text-center max-w-3xl mx-auto mb-12 sm:mb-16 space-y-4">
          <Badge variant="violet" size="md" icon={<Briefcase className="w-3.5 h-3.5 shrink-0" />}>
            Work History
          </Badge>
              ...
              return (
                ...
                >
                  ...
                  <div className="absolute left-1/2 top-8 -translate-x-1/2 z-20 hidden md:flex items-center justify-center">
                    <div className="w-8 h-8 rounded-full border-4 bg-sky-500 border-[var(--bg-page)] shadow-md flex items-center justify-center text-white text-xs font-bold shrink-0">
                      {index + 1}
                    </div>
                  </div>
                  ...
                  <div className={`w-full md:w-1/2 ${isEven ? 'md:pr-12 md:text-right' : 'md:pl-12 md:ml-auto'}`}>
                    <GlassCard className="p-5 sm:p-8 space-y-5 text-left border-[var(--border-subtle)] hover:border-sky-500/40">
                      
                      {/* Top Meta Bar */}
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[var(--border-subtle)] pb-3">
                        <div className="space-y-1">
                          <h3 className="text-lg sm:text-xl font-bold text-[var(--text-heading)] tracking-tight">{exp.position}</h3>
                          <div className="text-xs sm:text-sm font-semibold text-sky-600 dark:text-sky-400 flex items-center gap-1">
                            {exp.company}
                          </div>
                        </div>

                        <div className="flex flex-wrap items-center sm:flex-col sm:items-end gap-1.5 text-xs text-[var(--text-muted)] font-mono">
                          <span className="flex items-center gap-1 bg-[var(--bg-pill)] px-2.5 py-1 rounded-md border border-[var(--border-subtle)]">
                            <Calendar className="w-3 h-3 text-sky-500 shrink-0" /> {exp.period}
                          </span>
                          <span className="flex items-center gap-1">
                            <MapPin className="w-3 h-3 text-[var(--text-muted)] shrink-0" /> {exp.location}
                          </span>
                        </div>
                      </div>
                      ...
                      {exp.achievements && exp.achievements.length > 0 && (
                        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 space-y-1.5">
                          <div className="flex items-center gap-1.5 text-xs font-bold text-amber-600 dark:text-amber-400">
                            <Award className="w-3.5 h-3.5 shrink-0" /> Key Achievements
                          </div>
